...
func _on_push_pressed():
	...
	var nickname = str(Network.peer.get_unique_id()) if nickname_edit.text.is_empty() else nickname_edit.text
	Network.send_packet(PacketManager.create_packet(\
			PacketManager.PacketTypes.send_message_request,\
			create_message(nickname, message_edit.text)), 1)
