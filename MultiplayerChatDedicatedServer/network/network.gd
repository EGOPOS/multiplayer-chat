...
var peers_connected: Array[int] = []

func create_server():
	peer = WebSocketMultiplayerPeer.new()
	var ok = peer.create_server(port, address)
	print("server created succefful" if ok == OK else "server created failed")
	_bind_peer_signals()
...
func _peer_connected(peer):
	peers_connected.append(peer)
	var message = ChatManager.create_message(ChatManager.server_nickname, str(peer) + " connected")
	ChatManager.add_message(message)

func _peer_disconnected(peer):
	peers_connected.erase(peer)
	var message = ChatManager.create_message(ChatManager.server_nickname, str(peer) + " disconnected")
	ChatManager.add_message(message)
